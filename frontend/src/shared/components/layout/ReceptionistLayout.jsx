...
import {
  Menu, X, LogOut, User, Bell, Stethoscope, Home, Calendar, ClipboardList,
  ChevronRight, Mail
} from 'lucide-react';
...
const navItems = [
  ...
  { label: 'Quản lý lịch hẹn', href: `${APP_ROUTES.receptionistRoot}/lich-hen`, icon: <Calendar className="w-4 h-4" /> },
  { label: 'Đặt lịch khám', href: `${APP_ROUTES.receptionistRoot}/dat-lich`, icon: <ClipboardList className="w-4 h-4" /> },
  { label: 'Lịch bác sĩ', href: `${APP_ROUTES.receptionistRoot}/lich-bac-si`, icon: <Stethoscope className="w-4 h-4" /> },
  { label: 'Tin nhắn', href: `${APP_ROUTES.receptionistRoot}/tin-nhan`, icon: <Mail className="w-4 h-4" /> },
  { label: 'Thông tin cá nhân', href: `${APP_ROUTES.receptionistRoot}/thong-tin-ca-nhan`, icon: <User className="w-4 h-4" /> },
];
      ...
      <div className="p-3 border-b border-gray-100">
        <div className="flex items-center gap-3 px-2 py-2 rounded-lg bg-cyan-50">
          ...
          <div className="min-w-0">
            <div className="text-sm font-medium text-gray-900 truncate">{authUser?.name || 'Lễ tân'}</div>
            <div className="text-xs text-cyan-600">Lễ tân</div>
          </div>
        </div>
      </div>
